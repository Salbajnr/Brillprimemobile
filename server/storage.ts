import { 
  ...
  wallets, paymentMethods, walletTransactions,
  ...
  type Wallet, type InsertWallet, type PaymentMethod, type InsertPaymentMethod,
...
export interface IStorage {
  ...
  updatePaymentMethod(paymentMethodId: number, data: Partial<PaymentMethod>): Promise<PaymentMethod>;
  ...
  getWalletTransactions(userId: number, limit?: number, offset?: number): Promise<WalletTransaction[]>;
  getWalletTransaction(transactionId: string): Promise<WalletTransaction | undefined>;
  updateWalletTransactionStatus(transactionId: string, status: string, gatewayResponse?: any): Promise<WalletTransaction>;
  
  // Payment gateway operations
  processPayment(userId: number, amount: number, paymentMethodId: number, description: string): Promise<{ success: boolean; transactionId?: string; error?: string }>;
  fundWallet(userId: number, amount: number, paymentMethodId: number): Promise<{ success: boolean; transactionId?: string; error?: string }>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  // Wallet transaction operations implementation
  ...
  async getWalletTransactions(userId: number, limit: number = 50, offset: number = 0): Promise<WalletTransaction[]> {
  ...
  async getWalletTransaction(transactionId: string): Promise<WalletTransaction | undefined> {
  ...
  async updateWalletTransactionStatus(transactionId: string, status: string, gatewayResponse?: any): Promise<WalletTransaction> {
  ...
  // Payment gateway operations implementation
  async processPayment(userId: number, amount: number, paymentMethodId: number, description: string): Promise<{ success: boolean; transactionId?: string; error?: string }> {
    try {
      // Get payment method
      ...
      // Integrate with actual payment gateway based on payment method type
      ...
      try {
        switch (paymentMethod.type) {
          ...
          case 'visa':
            ...
            gatewayResponse = await this.processStripePayment(paymentMethod, amount, description);
            paymentSuccess = gatewayResponse.status === 'succeeded';
            break;
            
          case 'paypal':
            // PayPal integration
            gatewayResponse = await this.processPayPalPayment(paymentMethod, amount, description);
            paymentSuccess = gatewayResponse.status === 'COMPLETED';
            break;
            
          case 'apple_pay':
          case 'google_pay':
            // Mobile payment integration (typically through Stripe or similar)
            gatewayResponse = await this.processMobilePayment(paymentMethod, amount, description);
            paymentSuccess = gatewayResponse.status === 'succeeded';
            break;
            
          case 'bank_transfer':
            // Bank transfer integration (ACH, wire transfer, etc.)
            gatewayResponse = await this.processBankTransfer(paymentMethod, amount, description);
            paymentSuccess = gatewayResponse.status === 'pending'; // Bank transfers are usually pending initially
            break;
        ...
        await this.updateWalletTransactionStatus(transactionId, status, gatewayResponse);
        
        return { 
          success: paymentSuccess, 
          transactionId,
          error: paymentSuccess ? undefined : gatewayResponse.error || 'Payment failed'
        };
        
      } catch (error) {
        await this.updateWalletTransactionStatus(transactionId, 'failed', {
          error: error instanceof Error ? error.message : 'Unknown error'
        });
        return { success: false, error: 'Payment gateway error', transactionId };
      }
    } catch (error) {
  ...
  async fundWallet(userId: number, amount: number, paymentMethodId: number): Promise<{ success: boolean; transactionId?: string; error?: string }> {
    try {
      // Process payment first
      const paymentResult = await this.processPayment(userId, amount, paymentMethodId, `Wallet funding - $${amount}`);
      ...
      const fundingTransactionId = `fund_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      await this.createWalletTransaction({
        id: fundingTransactionId,
        userId,
        type: 'credit',
        amount,
        description: `Wallet funded via ${paymentResult.transactionId}`,
        status: 'completed',
        paymentMethodId
      });

      return { success: true, transactionId: fundingTransactionId };
    } catch (error) {
      return { success: false, error: 'Wallet funding error' };
    }
  }

  // Payment gateway integration methods
  private async processStripePayment(paymentMethod: PaymentMethod, amount: number, description: string): Promise<any> {
    ...
    return {
      ...
      currency: 'usd',
      description,
      payment_method: paymentMethod.details,
      error: success ? undefined : 'Your card was declined.'
    };
  }

  private async processPayPalPayment(paymentMethod: PaymentMethod, amount: number, description: string): Promise<any> {
  ...
  private async processMobilePayment(paymentMethod: PaymentMethod, amount: number, description: string): Promise<any> {
    // Apple Pay/Google Pay integration (usually through Stripe)
    const success = Math.random() > 0.1;
    return {
      ...
      error: success ? undefined : 'Mobile payment failed.'
    };
  }

  private async processBankTransfer(paymentMethod: PaymentMethod, amount: number, description: string): Promise<any> {
    // Bank transfer integration (ACH, wire transfer)
    return {
      ...
      currency: 'USD',
      description,
      bank_details: paymentMethod.details,
